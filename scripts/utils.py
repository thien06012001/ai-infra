...
def extract_wikilinks(content: str) -> list[str]:
    ...
    (`…`) so that syntax examples documenting the wikilink pattern don't get
    misreported as real (and therefore "broken") links.
    """
    # Strip fenced code blocks first — they can span multiple lines.
    without_fenced = re.sub(r"```.*?```", "", content, flags=re.DOTALL)
    # Strip inline code spans on every remaining line.
    without_inline = re.sub(r"`[^`\n]*`", "", without_fenced)
    return re.findall(r"\[\[([^\]]+)\]\]", without_inline)


def wiki_article_exists(link: str) -> bool:
    """Check if a wikilinked article exists on disk.

    Strips the optional Obsidian alias suffix (``[[path|display]]``) and any
    intra-page anchor (``[[path#heading]]``) before resolving — only the
    path before ``|`` and ``#`` identifies the target file. Anchor presence
    is not separately validated; the linter only confirms the article file
    exists.
    """
